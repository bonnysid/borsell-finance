...
@Injectable()
export class MoexEtfService {
  private readonly logger = new Logger(MoexEtfService.name);
  private readonly baseUrl = 'https://iss.moex.com';
  private readonly board = 'TQIF';
  ...
  public readonly ASSET_TYPE = AssetType.ETF;
  ...
  async getRequest<T>(url: string) {
    try {
      ...
      this.logger.error('Request error:', e.status);
      return null;
    }
  }

  async getEtfsInfo(tickers: string[]): Promise<MoexAssetInfo[]> {
  ...
  async getEtfInfo(ticker: string): Promise<MoexAssetInfo | null> {
    ...
    try {
      ...
      return {
        ...
        close: this.mapper.mapPrice(marketData?.CLOSEPRICE || marketData?.MARKETPRICE),
  ...
  async getTopEtfs(): Promise<MoexAssetInfo[]> {
    this.logger.log('Fetching TOP etfs');
