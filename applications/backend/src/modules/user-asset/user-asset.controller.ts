...
export class UserAssetController {
  ...
  async getBySymbol(
    ...
    const userAsset = await this.userAssetService.getUserAssetBySymbol(user.userId, symbol);

    if (!userAsset) {
      throw new NotFoundException(`Asset with symbol ${symbol} not found for this user`);
    }
    ...
    return res.status(200).json(new UserAssetDto(userAsset));
  }
